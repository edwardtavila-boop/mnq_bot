...
def main() -> int:
    ...
    if probe.get("available"):
        ...
        if bar is not None:
            snapshot = run_apex_evaluation(
                bar, setup, regime="NEUTRAL", **EVAL_EXTRA_KWARGS
            )
    ...
    if not probe.get("available"):
        ...
        lines.append(f"**Voices exposed:** {probe.get('voices_found', 0)} · "
    ...
    # Console line
    status = "🟢 LIVE" if snapshot is not None else (
        "🟡 ADAPTER-OK · engine unavailable" if not probe.get("available") else "🟡 ENGINE-OK · evaluate returned None"
    )

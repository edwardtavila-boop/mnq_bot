...
from mnq.features import ATR, EMA, VWAP, HTFWrapper, RelativeVolume
from mnq.generators.python_exec.base import BarCtx, StrategyBase
...
def _crossed(left_spec, right_spec, within, ctx):
    return _crossed_above(left_spec, right_spec, within, ctx) or _crossed_below(
        left_spec, right_spec, within, ctx
    )
...
class GeneratedStrategy(StrategyBase):
    ...
    def _eval_long(self, ctx: BarCtx) -> bool:
        try:
            return bool(
                (ctx.f("ema_fast") > ctx.f("ema_slow"))
                and _crossed_above(("f", "ema_fast"), ("f", "ema_slow"), 3, ctx)
                and (float(ctx.bar.close) > ctx.f("vwap_session"))
                and ctx.hist("htf_trend").rising(2)
                and (ctx.f("rvol_20") > 1.2)
                and (ctx.session_window in ("rth_open_drive", "afternoon"))
                and (not ctx.in_blackout)
                and (ctx.position_size == 0)
            )
        except RuntimeError:
    ...
    def _eval_short(self, ctx: BarCtx) -> bool:
        try:
            return bool(
                (ctx.f("ema_fast") < ctx.f("ema_slow"))
                and _crossed_below(("f", "ema_fast"), ("f", "ema_slow"), 3, ctx)
                and (float(ctx.bar.close) < ctx.f("vwap_session"))
                and ctx.hist("htf_trend").falling(2)
                and (ctx.f("rvol_20") < 1.2)
                and (ctx.session_window in ("rth_open_drive", "afternoon"))
                and (not ctx.in_blackout)
                and (ctx.position_size == 0)
...
def build(spec) -> GeneratedStrategy:
    ...
    if spec.strategy.content_hash and SPEC_HASH and spec.strategy.content_hash != SPEC_HASH:
        raise ValueError(f"spec hash {spec.strategy.content_hash} != generator hash {SPEC_HASH}")
    features = {

...
def _voices_all_agree(direction: int = 1, *, fire_long: bool = True,
                      pm_final: float = 55.0) -> dict:
...
def _run(spec: dict, *, regime: dict | None = None, voices: dict | None = None,
         pm_final: float | None = None) -> dict:
    payload: dict = {
        'spec': spec,
        'side': spec['side'],
        'market_state': {
...
class TestRedTeamMandatoryUnderApex:
    ...
    def test_apex_enrichment_does_not_erase_agent_outputs(self):
        ...
        assert set(all_verdicts.keys()) == {'quant', 'red_team', 'risk',
                                             'macro', 'micro'}

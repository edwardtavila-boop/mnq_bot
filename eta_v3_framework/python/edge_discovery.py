...
def tod_bucket(ts):
    ...
    if m < 9 * 60 + 30:
        return "premarket"
    if m < 10 * 60 + 30:
        return "open_30min"  # 9:30-10:30
    if m < 11 * 60 + 30:
        return "mid_am"  # 10:30-11:30
    if m < 13 * 60 + 30:
        return "lunch"  # 11:30-13:30
    if m < 14 * 60 + 30:
        return "early_pm"  # 13:30-14:30
    if m < 15 * 60 + 30:
        return "power_hour"  # 14:30-15:30
    if m < 16 * 60:
        return "moc"  # 15:30-16:00
    return "after_hours"
...
def aggregate_bucket(trades, group_fn, min_n=5):
    ...
    for key, ts in buckets.items():
        ...
        avg_mfe = (
            statistics.mean(t.mfe_R for t in ts) if all(hasattr(t, "mfe_R") for t in ts) else 0
        )
        avg_mae = (
            statistics.mean(t.mae_R for t in ts) if all(hasattr(t, "mae_R") for t in ts) else 0
        )
        results.append(
            {
                "bucket": str(key),
...
def write_trades_csv(trades, path):
    ...
    with open(path, "w", newline="") as f:
        ...
        writer.writeheader()
        for t in trades:
            et_dt = datetime.fromtimestamp(t.open_time, tz=UTC).astimezone(ET)
            row = {
...
def write_findings_md(
    trades, by_setup, by_tod, by_regime, by_setup_tod, by_voice_sig, exp_analysis, total_r, pm
):
    ...
    lines.append("")
    lines.append("| Setup | n | Win% | Strike% | Avg R | Total R | PF | Avg MFE | Avg MAE |")
    lines.append("|-------|---|------|---------|-------|---------|-----|---------|---------|")
    for r in by_setup:
        lines.append(
            f"| {r['bucket']} | {r['n']} | {r['win_rate']} | {r['strike']} | {r['avg_r']:+.3f} | {r['total_r']:+.2f} | {r['pf']} | {r['avg_mfe']} | {r['avg_mae']} |"
        )
    lines.append("")

    lines.append("## By Time of Day (RTH only, excluding pre/after market)")
    lines.append("")
    lines.append("| TOD | n | Win% | Strike% | Avg R | Total R | PF |")
    lines.append("|-----|---|------|---------|-------|---------|-----|")
    for r in by_tod:
        if r["bucket"] in ("weekend", "premarket", "after_hours"):
            continue
        lines.append(
            f"| {r['bucket']} | {r['n']} | {r['win_rate']} | {r['strike']} | {r['avg_r']:+.3f} | {r['total_r']:+.2f} | {r['pf']} |"
        )
    lines.append("")

    lines.append("## By Regime")
    lines.append("")
    lines.append("| Regime | n | Win% | Strike% | Avg R | Total R | PF |")
    lines.append("|--------|---|------|---------|-------|---------|-----|")
    for r in by_regime:
        lines.append(
            f"| {r['bucket']} | {r['n']} | {r['win_rate']} | {r['strike']} | {r['avg_r']:+.3f} | {r['total_r']:+.2f} | {r['pf']} |"
    ...
    lines.append("## TOP 15 Setup × Time-of-Day Combinations (positive expectancy, n>=5)")
    lines.append("")
    lines.append("| Bucket | n | Win% | Strike% | Avg R | Total R | PF |")
    lines.append("|--------|---|------|---------|-------|---------|-----|")
    ...
    lines.append("## BOTTOM 10 Setup × TOD (negative expectancy — avoid these)")
    lines.append("")
    lines.append("| Bucket | n | Win% | Strike% | Avg R | Total R | PF |")
    lines.append("|--------|---|------|---------|-------|---------|-----|")
    ...
    lines.append("## TOP 10 Voice Signatures (pattern of voices firing together)")
    lines.append("")
    lines.append("| Voice signature | n | Win% | Strike% | Avg R | Total R |")
    lines.append("|-----------------|---|------|---------|-------|---------|")
    for r in by_voice_sig[:10]:
        lines.append(
            f"| {r['bucket']} | {r['n']} | {r['win_rate']} | {r['strike']} | {r['avg_r']:+.3f} | {r['total_r']:+.2f} |"
        )
    lines.append("")

    lines.append("## Expiration Deep Dive")
    lines.append("")
    if exp_analysis:
        lines.append(
            f"- Total expired trades: {exp_analysis['n_expired']} ({exp_analysis['pct_of_total']}% of all trades)"
        )
        lines.append(f"- Avg MFE while open: {exp_analysis['avg_mfe_R']}R")
        lines.append(f"- Avg MAE while open: {exp_analysis['avg_mae_R']}R")
        lines.append(
            f"- Expired trades that hit MFE >= 0.5R: {exp_analysis['n_with_mfe_above_0.5R']}"
        )
        lines.append(
            f"- Expired trades that hit MFE >= 1.0R: {exp_analysis['n_with_mfe_above_1.0R']}"
        )
        lines.append(
            f"- **Potential R recovered with 0.5R partial-take rule**: +{exp_analysis['potential_R_saved_at_0.5R']}R"
        )
        lines.append(
            f"- **Potential R recovered with 1.0R partial-take rule**: +{exp_analysis['potential_R_saved_at_1.0R']}R"
        )
    lines.append("")
    ...
    best_tod = next(
        (
            r
            for r in by_tod
            if r["bucket"] not in ("weekend", "premarket", "after_hours") and r["avg_r"] > 0
        ),
        None,
    )
    ...
    if best_setup:
        lines.append(
            f"- Best-performing setup: **{best_setup['bucket']}** at {best_setup['avg_r']:+.3f}R/trade ({best_setup['n']} trades)"
        )
    if best_tod:
        lines.append(
            f"- Best time-of-day: **{best_tod['bucket']}** at {best_tod['avg_r']:+.3f}R/trade ({best_tod['n']} trades)"
        )
    if best_combo:
        lines.append(
            f"- Best setup × TOD combination with statistical weight: **{best_combo['bucket']}** at {best_combo['avg_r']:+.3f}R/trade ({best_combo['n']} trades, {best_combo['strike']}% strike)"
        )
    lines.append("")
    if exp_analysis and exp_analysis["potential_R_saved_at_0.5R"] > 1:
        lines.append(
            f"- **Time-stop opportunity**: A 0.5R partial-take rule on stalled trades would recover +{exp_analysis['potential_R_saved_at_0.5R']}R that V1 currently leaves on the table"
        )
    lines.append("")
    ...
    lines.append("That replaces the PM gate with data-derived confluence.")
...
def main():
    ...
    print(
        f"  {len(bars):,} bars  ({datetime.fromtimestamp(bars[0].time, tz=UTC):%Y-%m-%d} → {datetime.fromtimestamp(bars[-1].time, tz=UTC):%Y-%m-%d})"
    )
    ...
    by_setup_tod = aggregate_bucket(
        trades, lambda t: f"{t.setup}_{tod_bucket(t.open_time)}", min_n=5
    )
    ...
    print("\nBy Setup:")
    print(
        f"  {'Setup':<10s} {'n':>5s} {'Win%':>6s} {'Strike':>7s} {'AvgR':>7s} {'TotR':>7s} {'PF':>6s}"
    )
    for r in by_setup:
        print(
            f"  {r['bucket']:<10s} {r['n']:>5d} {r['win_rate']:>5.1f}% {r['strike']:>6.1f}% {r['avg_r']:>+7.3f} {r['total_r']:>+7.2f} {str(r['pf']):>6s}"
        )

    print("\nBy Time of Day (RTH):")
    print(
        f"  {'TOD':<14s} {'n':>5s} {'Win%':>6s} {'Strike':>7s} {'AvgR':>7s} {'TotR':>7s} {'PF':>6s}"
    )
    for r in by_tod:
        if r["bucket"] in ("weekend", "premarket", "after_hours"):
            continue
        print(
            f"  {r['bucket']:<14s} {r['n']:>5d} {r['win_rate']:>5.1f}% {r['strike']:>6.1f}% {r['avg_r']:>+7.3f} {r['total_r']:>+7.2f} {str(r['pf']):>6s}"
        )

    print("\nBy Regime:")
    print(
        f"  {'Regime':<12s} {'n':>5s} {'Win%':>6s} {'Strike':>7s} {'AvgR':>7s} {'TotR':>7s} {'PF':>6s}"
    )
    for r in by_regime:
        print(
            f"  {r['bucket']:<12s} {r['n']:>5d} {r['win_rate']:>5.1f}% {r['strike']:>6.1f}% {r['avg_r']:>+7.3f} {r['total_r']:>+7.2f} {str(r['pf']):>6s}"
        )
    ...
    for r in by_setup_tod:
        if r["avg_r"] <= 0 or shown >= 10:
            continue
        print(
            f"  {r['bucket']:<22s} {r['n']:>5d} {r['strike']:>6.1f}% {r['avg_r']:>+7.3f} {r['total_r']:>+7.2f}"
        )
        shown += 1
    ...
    findings_md = write_findings_md(
        trades, by_setup, by_tod, by_regime, by_setup_tod, by_voice_sig, exp, total_r, args.pm
    )

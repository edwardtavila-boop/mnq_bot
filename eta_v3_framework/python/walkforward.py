...
from firm_engine import Bar, FirmConfig
...
def run_window(bars, cfg, det_cfg):
    ...
    if bars:
        s["start"] = datetime.fromtimestamp(bars[0].time, tz=UTC).strftime("%Y-%m-%d")
        s["end"] = datetime.fromtimestamp(bars[-1].time, tz=UTC).strftime("%Y-%m-%d")
    return s
...
def aggregate(results, pm):
    ...
    avg_wr = (
        statistics.mean(r["win_rate"] for r in results if r.get("trades", 0) > 0)
        if n_with > 0
        else 0
    )
    return {
...
def main():
    ...
    if args.sweep:
        ...
        print(
            f"{'PM':>4s}  {'Win%':>6s}  {'Trades':>7s}  {'TotR':>8s}  {'Profit':>14s}  {'AvgWR':>6s}"
        )
        print("-" * 60)
        sweep_results = []
        for pm in [25, 30, 35, 40, 45, 50]:
            ...
            print(
                f"{pm:>4d}  {agg['avg_win_rate']:>5.1f}%  {agg['total_trades']:>7d}  "
                f"{agg['total_r']:>+8.2f}  "
        ...
        for i, win_bars in enumerate(windows, 1):
            ...
            n = r.get("trades", 0)
            print(
                f"  Window {i}/{len(windows)}: {n} trades  "
    ...
    print(f"{'=' * 78}")
    print(
        f"{'#':>3s}  {'Range':>23s}  {'Trades':>7s}  {'Win%':>6s}  {'TotR':>7s}  {'PF':>6s}  {'MDD':>6s}  {'Status':>10s}"
    )
    print("─" * 78)

...
def main():
    ...
    print(f"\n{'─' * 80}")
    print("FULL v2 BY SETUP")
    print(f"{'─' * 80}")
    for setup, st in s.get("by_setup", {}).items():
        print(
            f"  {setup:8s}: {st['trades']:3d} trades, {st['win_rate'] * 100:5.1f}% win, total {st['total_r']:+.1f}R"
        )

    print(f"\n{'─' * 80}")
    print("FULL v2 BY REGIME")
    print(f"{'─' * 80}")
    ...
    if bt.equity_curve:
        ...
        if n >= 5:
            ...
            print("\n  Curve samples:")
            for t, r in samples:
                dt = datetime.fromtimestamp(t, tz=UTC).strftime("%Y-%m-%d %H:%M")
                print(f"    {dt}  R={r:+.2f}")

...
def run_tf_backtest(csv_path, tf, pm, vix=None, es=None, dxy=None, tick=None, verbose=False):
    ...
    if verbose:
        ...
        print(f"{'=' * 70}")
        if bars:
            print(f"  Start: {datetime.fromtimestamp(bars[0].time, tz=UTC).strftime('%Y-%m-%d')}")
            print(f"  End:   {datetime.fromtimestamp(bars[-1].time, tz=UTC).strftime('%Y-%m-%d')}")
        if s.get("trades", 0) == 0:
            print(f"  No trades fired. Decisions: {s.get('decisions', 0)}")
            return s, bt
        print(
            f"  Trades:       {s['trades']}  (W:{s['wins']} L:{s['losses']} BE:{s.get('breakevens', 0)})"
        )
        print(
            f"  Strike rate:  {s['wins'] / (s['wins'] + s['losses']) * 100:.1f}%"
        ...
        for setup, st in s["by_setup"].items():
            print(
                f"    {setup:8s}: {st['trades']:3d} trades, {st['win_rate'] * 100:4.1f}% win, {st['total_r']:+.1f}R"
            )
...
def main():
    ...
    if args.compare_all:
        ...
        results = []
        for tf, csv_name in [
            ("5m", args.csv.replace("_15m", "_5m").replace("_1h", "_5m")),
        ...
        print(f"{'=' * 90}")
        print(
            f"{'TF':>4s}  {'Bars':>6s}  {'Trades':>7s}  {'Wins':>5s}  {'Strike':>7s}  {'TotR':>8s}  {'PF':>5s}  {'MDD':>5s}"
        )
        print("-" * 70)
        for tf, csv_name, s in results:
            if s.get("trades", 0) > 0:
                ...
                print(
                    f"{tf:>4s}  {s.get('bars', 0):>6d}  {s['trades']:>7d}  {w:>5d}  "

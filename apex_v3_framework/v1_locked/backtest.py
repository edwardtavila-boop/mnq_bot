...
class V1DetectorConfig:
    ...
    ema_dow_filter: str = "Skip Thursday"
    ...
    # Entry refinement
    entry_mode: str = (
        "market"  # "market" or "pullback" — pullback uses limit at signal_close - pullback_atr*ATR
    )
    ...
    # "r_multiple": fixed TP1/TP2 at R-multiples (default, preserves v1 behavior)
    # "alligator":  exit on Lips cross-back (best for trend-following EMA PB)
    # "fibonacci":  TP at Fib extensions of underlying swing (best for ORB/Sweep structure plays)
    # "hybrid":     setup-specific best — Alligator for EMA PB, Fibonacci for ORB/Sweep
    exit_mode: str = "r_multiple"
...
class V1Detector:
    ...
    last_swing_hi: float | None = None
    last_swing_lo: float | None = None
    bos_bull_active: bool = False
    bos_bear_active: bool = False
    bos_bull_bar: int = -999
    bos_bear_bar: int = -999
    ob_demand_hi: float | None = None
    ob_demand_lo: float | None = None
    ob_supply_hi: float | None = None
    ob_supply_lo: float | None = None
    reclaim_up_armed: bool = False
    reclaim_dn_armed: bool = False
    reclaim_up_bar: int = -999
    reclaim_dn_bar: int = -999
    swept_lo_px: float | None = None
    swept_hi_px: float | None = None
    ...
    rth_vol_count: int = 0
    today_open_px: float | None = None

    def _new_day(self):
        self.or_high = None
        self.or_low = None
        self.or_set = False
        self.orb_long_triggered = False
        self.orb_short_triggered = False
        self.or_broke_up = False
        self.or_broke_dn = False
        self.bos_bull_active = False
        self.bos_bear_active = False
        self.rth_vol_sum = 0.0
        self.rth_vol_count = 0
    ...
    def _ema_tod_ok(self, bar_dt):
        ...
        if f == "Avoid Lunch":
            return not (11 * 60 + 30 <= m < 13 * 60 + 30)
        if f == "Power Hours":
    ...
    def _ema_dow_ok(self, bar_dt):
        ...
        if f == "Skip Thursday":
            return dow != 4
        if f == "Skip Thu+Fri":
            return dow != 4 and dow != 5
        if f == "Mon-Tue Only":
            return dow == 1 or dow == 2
        return True

    def _orb_bias_long(self):
        if not self.cfg.ema_orb_align:
            return True
        if not self.or_set:
            return True
    ...
    def _orb_score(self, is_long, bar, vol_z, near_key):
        ...
        if bar.htf_close is not None and bar.htf_ema50 is not None:
            if (is_long and bar.htf_close > bar.htf_ema50) or (
                not is_long and bar.htf_close < bar.htf_ema50
            ):
                s += 1
        if vol_z >= 1.5:
            s += 1
        if bar.adx is not None and bar.adx >= 20:
            s += 1
    ...
    def _ema_score(self, is_long, bar, has_rejection, near_key, vol_z):
        ...
        if bar.adx is not None and bar.adx >= 22:
            s += 1
        if has_rejection:
            s += 1
        if bar.atr is not None and bar.ema21 is not None:
            if (
                is_long
                and bar.low > bar.ema21 - bar.atr * 0.3
                or not is_long
                and bar.high < bar.ema21 + bar.atr * 0.3
            ):
                s += 1
        if near_key:
            s += 1
    ...
    def _sweep_score(self, is_long, bar, vol_z, near_key):
        ...
        if near_key:
            s += 1
        if bar.adx is not None and bar.adx <= 25:
            s += 1
        if bar.htf_close is not None and bar.htf_ema50 is not None:
            if (is_long and bar.htf_close > bar.htf_ema50) or (
                not is_long and bar.htf_close < bar.htf_ema50
    ...
    def detect(self, bar_idx, bar, state):
        ...
        vol_ma = (
            (self.rth_vol_sum / self.rth_vol_count)
            if self.rth_vol_count > 5
            else (
                sum(state._vol_history) / len(state._vol_history)
                if state._vol_history
                else bar.volume
            )
        )
        ...
        vol_z = state.vol_z()
        near_key = (
            self._f_near(state.prev_day_high, bar.close, bar.atr or 0)
        ...
        orb_pending = (
            self.or_set
            and in_session
            and not self.orb_long_triggered
            and not self.orb_short_triggered
        )
        ...
        if (
            bar.ema9 is not None
            and bar.ema21 is not None
            ...
            if trend_bull and not self.was_bull:
                self.pb_count_bull = 0
            if trend_bear and not self.was_bear:
                self.pb_count_bear = 0
            if in_zone and not self.was_in_zone:
                if trend_bull:
                    self.pb_count_bull += 1
                if trend_bear:
                    self.pb_count_bear += 1
            ...
            pin_bull = (bar.open - bar.low) > (bar.high - bar.close) * 2 and (
                bar.high - bar.low
            ) > bar.atr * 0.8
            pin_bear = (bar.high - bar.open) > (bar.close - bar.low) * 2 and (
                bar.high - bar.low
            ) > bar.atr * 0.8
            ...
            adx_ok = self.cfg.ema_adx_floor == 0 or (
                bar.adx is not None and bar.adx >= self.cfg.ema_adx_floor
            )
        ...
        if (
            self.last_swing_hi is not None
            ...
            for i in range(min(10, len(self.close_history))):
                idx = -1 - i
                if abs(idx) > len(self.close_history):
                    break
                if self.close_history[idx] < self.open_history[idx]:
                    ...
                    break
        if (
            self.last_swing_lo is not None
            and len(self.close_history) > 0
        ...
        if (
            self.bos_bull_active
            and self.last_swing_lo is not None
            and bar.atr
            and bar.low < self.last_swing_lo
            and bar.close > self.last_swing_lo
        ...
        if (
            self.bos_bear_active
            and self.last_swing_hi is not None
            and bar.atr
            and bar.high > self.last_swing_hi
            and bar.close < self.last_swing_hi
        ...
        min_body = 4 * TICK
        if (
            self.reclaim_up_armed
            and self.ob_demand_lo is not None
            and bar.close > self.ob_demand_lo
            ...
            self.reclaim_up_armed = False
        if (
            self.reclaim_dn_armed
            and self.ob_supply_hi is not None
            and bar.close < self.ob_supply_hi
            and bar.close < bar.open
            and abs(bar.close - bar.open) >= min_body
        ):
        ...
        if (
            1 <= bar_idx - self.reclaim_up_bar <= self.cfg.sweep_entry_win
            and bar.close > bar.open
            and cd_sweep
            and in_session
            and len(self.close_history) > 0
        ...
        if (
            1 <= bar_idx - self.reclaim_dn_bar <= self.cfg.sweep_entry_win
            and bar.close < bar.open
            and cd_sweep
            and in_session
            and len(self.close_history) > 0
...
class Trade:
    ...
    pullback_filled_at: float | None = None  # If pullback entry, actual fill price
...
class Backtester:
    ...
    micro_refiner: object | None = None
    ...
    def _process_pending_limits(self, bar, idx):
        ...
        for p in self.pending_limits:
            ...
            if (
                p.side == "long"
                and bar.low <= p.limit_price
                or p.side == "short"
                and bar.high >= p.limit_price
            ):
                filled = True
    ...
    def _execute_trade(
        self, bar, idx, d, side, setup, fill_price, size_mult, pullback_filled=False, micro_sl=None
    ):
        ...
        if setup == "ORB":
            ...
            if side == "long" and st_or_low is not None:
                ...
                sl = (
                    fill_price - (bar.atr or 0) * 1.5
                    if side == "long"
                    else fill_price + (bar.atr or 0) * 1.5
                )

            if (
                use_fib
                and st_or_low is not None
                and st_or_high is not None
                and (st_or_high - st_or_low) > (bar.atr or 0) * 0.5
            ):  # min meaningful range
                ...
                tp1_r = (
                    abs(tp1 - fill_price) / abs(fill_price - sl)
                    if abs(fill_price - sl) > 0
                    else 1.5
                )
                tp2_r = (
                    abs(tp2 - fill_price) / abs(fill_price - sl)
                    if abs(fill_price - sl) > 0
                    else 3.0
                )
            else:
        ...
        t = Trade(
            ...
            voices=d.voices,
            regime=d.regime,
            size_pct=int(size_pct * size_mult),
    ...
    def _open_trade(self, bar, idx, d, st, size_mult: float = 1.0):
        ...
        if self.use_micro_entry and self.bars_1m and self.micro_refiner is not None:
            ...
            if next_1m:
                ...
                if setup == "ORB":
                    signal_sl = (
                        (st.or_low - (bar.atr or 0) * 0.15)
                        if side == "long" and st.or_low
                        else (st.or_high + (bar.atr or 0) * 0.15)
                        if st.or_high
                        else bar.close
                    )
                    or_h = st.or_high if st.or_high else bar.close
                    or_l = st.or_low if st.or_low else bar.close
                    micro = self.micro_refiner.refine_orb(
                        side, signal_entry, signal_sl, or_h, or_l, next_1m
                    )
                elif setup == "EMA PB":
                    signal_sl = (
                        bar.close - (bar.atr or 0) * 1.5
                        if side == "long"
                    ...
                    swept = (
                        self.detector.swept_lo_px if side == "long" else self.detector.swept_hi_px
                    )
                    if swept is None:
                        swept = bar.close
                    signal_sl = swept - 4 * TICK if side == "long" else swept + 4 * TICK
                    micro = self.micro_refiner.refine_sweep(
                        side, signal_entry, signal_sl, swept, next_1m
                    )
                else:
                    micro = None
        ...
        if self.detector_cfg.entry_mode == "pullback" and bar.atr is not None and setup != "ORB":
    ...
    def _manage_open(self, bar, idx):
        ...
        for t in self.open_trades:
            ...
            if (
                self.detector_cfg.use_mfe_trail
                and t.mfe_R >= self.detector_cfg.trail_arm_R
                and not t.tp1_filled
                and t.sl_dist > 0
            ):
                lock_price = (
                    t.entry + (t.sl_dist * self.detector_cfg.trail_lock_R)
                    if t.side == "long"
            ...
            mode_active_alligator = self.detector_cfg.exit_mode == "alligator" or (
            ...
            if (
                ...
                if (
                    t.side == "long"
                    and bar.close < bar.alligator_lips
                    or t.side == "short"
                    and bar.close > bar.alligator_lips
                ):
                ...
                if (
                    t.alligator_against_count >= self.detector_cfg.alligator_exit_bars
                    and t.mfe_R > 0.3
                ):
                    ...
                    t.close_idx = idx
                    t.close_time = bar.time
                    t.close_px = bar.close
            ...
            if self.detector_cfg.use_partials and t.tp1_filled and self.detector_cfg.use_runner:
                if bar.ema9 is not None and bar.atr is not None:
                    trail = (
                        bar.ema9 - bar.atr * 0.15 if t.side == "long" else bar.ema9 + bar.atr * 0.15
                    )
            ...
            if closed:
                ...
                if self.slip_per_trade_R > 0:
                    t.pnl_r = (
                        t.pnl_r - self.slip_per_trade_R
                        if t.pnl_r > 0
                        else t.pnl_r - self.slip_per_trade_R
                    )
                t.bars_to_resolution = held
                self.trades.append(t)
                self.daily_pnl_r += t.pnl_r
    ...
    def _summary(self):
        ...
        if not self.trades:
            return {
                ...
                "trades": 0,
                "decisions": n_dec,
                "decisions_with_direction": n_dir,
                "decisions_pm_passed": n_pm,
            }
...
def main():
    ...
    p.add_argument("csv")
    p.add_argument("--pm", type=float, default=35.0)
    ...
    p.add_argument("--entry-mode", default="market", choices=["market", "pullback"])
    p.add_argument(
        "--exit-mode",
        default="r_multiple",
    ...
    p.add_argument(
        "--no-circuit-breaker", action="store_true", help="Disable daily P&L circuit breaker"
    )
    ...
    if args.vix or args.es or args.dxy or args.tick:
        ...
        print(
            f"Loaded {len(bars)} bars  ({datetime.fromtimestamp(bars[0].time)} → {datetime.fromtimestamp(bars[-1].time)})"
        )
        print(
            f"Intermarket coverage: VIX {cov['with_vix']}/{cov['total_bars']}, "
    ...
    for setup, st in s["by_setup"].items():
        print(
            f"  {setup:8s}: {st['trades']:3d} trades  {st['win_rate'] * 100:5.1f}% win  avg {st['avg_r']:+.2f}R  total {st['total_r']:+.1f}R"
        )
    print("\n── By Regime ──")
    for reg, st in s["by_regime"].items():
        print(
            f"  {reg:10s}: {st['trades']:3d} trades  {st['win_rate'] * 100:5.1f}% win  total {st['total_r']:+.1f}R"
        )
    if args.verbose:
        print("\n── All Trades ──")
        for t in bt.trades:
            dt = datetime.fromtimestamp(t.open_time, tz=UTC).strftime("%Y-%m-%d %H:%M")
            print(
                f"  {dt}  {t.side:5s} {t.setup:6s}  PM={t.pm_final:5.1f}  Q={t.quant:+5.1f}  R={t.red:4.0f}  {t.regime:10s}  → {t.outcome:14s} {t.pnl_r:+.2f}R"
            )

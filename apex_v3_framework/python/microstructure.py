...
class MicroEntryRefiner:
    ...
    # ─────────────────────────────────────────────────────────────────
    def refine_orb(self, side: str, signal_entry: float, signal_sl: float,
                   or_high: float, or_low: float,
                   next_1m_bars: List[MicroBar]) -> MicroEntry:
        ...
        if not next_1m_bars:
            return MicroEntry(False, signal_entry, signal_sl, 0.0, 0,
                             "no 1m data", 1.0)
        ...
        for i, b in enumerate(next_1m_bars[:self.max_wait]):
            ...
            if side == "long":
                ...
                if held_above and strong_close:
                    ...
                    return MicroEntry(True, entry, micro_sl, conf, i,
                                     f"1m confirmed above OR+strong close", refined_r)
            else:  # short
                ...
                if held_below and strong_close:
                    ...
                    return MicroEntry(True, entry, micro_sl, conf, i,
                                     "1m confirmed below OR+strong close", refined_r)
        # No confirmation within wait window — skip the trade
        return MicroEntry(False, signal_entry, signal_sl, 0.0, self.max_wait,
                         "no 1m confirmation within wait window", 1.0)

    # ─────────────────────────────────────────────────────────────────
    # EMA Pullback: 1m rejection candle within pullback zone
    # ─────────────────────────────────────────────────────────────────
    def refine_ema_pullback(self, side: str, signal_entry: float, signal_sl: float,
                             ema9: float, ema21: float, atr: float,
                             next_1m_bars: List[MicroBar]) -> MicroEntry:
        ...
        for i, b in enumerate(next_1m_bars[:self.max_wait]):
            ...
            if side == "long":
                ...
                if i > 0:
                    ...
                    is_engulf = (b.close > b.open and prev.close < prev.open
                                 and b.open < prev.close and b.close > prev.open)
                ...
                if (is_pin or is_engulf) and is_bear_close and near_ema:
                    ...
                    return MicroEntry(True, entry, micro_sl, 0.85, i, reason, refined_r)
        return MicroEntry(False, signal_entry, signal_sl, 0.0, self.max_wait,
                         "no 1m rejection candle", 1.0)

    # ─────────────────────────────────────────────────────────────────
    # Sweep: 1m re-test of swept level that holds (doesn't get swept again)
    # ─────────────────────────────────────────────────────────────────
    def refine_sweep(self, side: str, signal_entry: float, signal_sl: float,
                     swept_level: float, next_1m_bars: List[MicroBar]) -> MicroEntry:
        ...
        for i, b in enumerate(next_1m_bars[:self.max_wait]):
            ...
            if side == "long":
                ...
                if retest and holds:
                    ...
                    micro_r_dist = abs(entry - micro_sl)
                    refined_r = orig_r_dist / micro_r_dist if micro_r_dist > 0 else 1.0
                    return MicroEntry(True, entry, micro_sl, 0.9, i,
                                     "1m retest held above swept level", refined_r)
            else:  # short
                ...
                if retest and holds:
                    ...
                    refined_r = orig_r_dist / micro_r_dist if micro_r_dist > 0 else 1.0
                    return MicroEntry(True, entry, micro_sl, 0.9, i,
                                     "1m retest held below swept level", refined_r)
        return MicroEntry(False, signal_entry, signal_sl, 0.0, self.max_wait,
                         "no 1m retest within wait window", 1.0)
...
def get_1m_bars_in_5m_window(bars_1m: List[MicroBar], start_time: int,
                              n_bars: int = 5) -> List[MicroBar]:

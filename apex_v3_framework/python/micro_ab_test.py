...
def fmt_stats(label, s, bt):
    ...
    return (
        f"  [{label}] {s['trades']:>2d} trades  W:{w} L:{l} BE:{be}  "
...
def main():
    ...
    if args.vix or args.es or args.dxy or args.tick:
        bars_5m = load_with_intermarket(
            args.csv5, vix=args.vix, es=args.es, dxy=args.dxy, tick=args.tick
        )
    else:
    ...
    print(
        f"  5m bars: {len(bars_5m)} ({datetime.fromtimestamp(bars_5m[0].time, tz=UTC):%Y-%m-%d} → {datetime.fromtimestamp(bars_5m[-1].time, tz=UTC):%Y-%m-%d})"
    )
    print(
        f"  1m bars: {len(bars_1m)} ({datetime.fromtimestamp(bars_1m[0].time, tz=UTC):%Y-%m-%d} → {datetime.fromtimestamp(bars_1m[-1].time, tz=UTC):%Y-%m-%d})"
    )
    ...
    if bt_b.micro_r_improvements:
        ...
        print(
            f"  Min / max:               {min(bt_b.micro_r_improvements):.2f}× / {max(bt_b.micro_r_improvements):.2f}×"
        )
        print(f"  Signals skipped by micro: {bt_b.micro_skip_count}")
        print(
            f"  Signal acceptance rate:  {len(bt_b.micro_r_improvements) / (len(bt_b.micro_r_improvements) + bt_b.micro_skip_count) * 100:.0f}%"
            if (len(bt_b.micro_r_improvements) + bt_b.micro_skip_count) > 0
            else ""
        )

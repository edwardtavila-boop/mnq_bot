...
def main():
    ...
    p.add_argument("--resample", default="5m",
                   help="Target timeframe (default 5m, aggregated from 1m)")
    p.add_argument("--estimate", action="store_true",
                   help="Show cost estimates only, no fetch")
    ...
    if args.estimate:
        print(f"\n{'='*72}")
        print(f"ESTIMATE ONLY - no data fetched. Remove --estimate to proceed.")
        print(f"{'='*72}")
        return
    ...
    print(f"\nNext steps:")
    print(f"  # Run full backtest on historical NQ data")

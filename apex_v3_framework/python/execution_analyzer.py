...
def analyze(trades):
    ...
    for setup, ts in by_setup.items():
        ...
        pullback_avail = [abs(t.mae_R) > 0.2 for t in ts if t.outcome in ("tp1", "tp2", "tp1_then_be")]
        ...
        report[setup] = {
            ...
            "capture_ratio_p50": round(percentile(capture_ratios, 50) * 100, 1) if capture_ratios else 0,
...
def print_report(report):
    ...
    print("\nLegend:")
    print("  WinMAE   = Maximum Adverse Excursion on winners (how much they pulled back before going)")
...
def actionable(report):
    ...
    for setup, r in report.items():
        ...
        if r['winner_mae_p50'] < 0.2:
            print(f"  ✓ ENTRY QUALITY HIGH: median winner pulled back only {r['winner_mae_p50']}R "
                  f"after entry. Current entry timing is solid.")
        elif r['winner_mae_p50'] > 0.5:
            print(f"  ⚠ ENTRY TIMING: median winner pulled back {r['winner_mae_p50']}R before going. "
        ...
        if r['capture_ratio_p50'] < 50 and r['winner_mfe_p50'] > 1.5:
            print(f"  ⚠ EXIT TIMING: leaving {100 - r['capture_ratio_p50']:.0f}% of profit on the table. "
            ...
            print(f"  ✓ EXIT QUALITY: capturing {r['capture_ratio_p50']:.0f}% of peak MFE. Tight exits.")
        # Hold time guidance
        if r['winner_mfe_bars_p50'] > 0:
            print(f"  → OPTIMAL HOLD: median MFE peaks at bar {r['winner_mfe_bars_p50']} "
        ...
        if r['loser_mfe_p50'] > 0.5:
            print(f"  ⚠ LOSERS HAD CHANCES: median losing trade reached +{r['loser_mfe_p50']}R "
        ...
        if r['pullback_avail_pct'] > 60:
            print(f"  → PULLBACK ENTRIES: {r['pullback_avail_pct']:.0f}% of winners pulled back "
...
def main():
    ...
    s = bt.run(bars)
    print(f"Backtest complete: {s.get('trades', 0)} trades, "
    ...
    print("=" * 90)
    print(f"{'Time':>16s}  {'Setup':>6s}  {'Side':>5s}  {'Bars':>4s}  "
          f"{'MAE':>6s}  {'MFE':>6s}  {'@Bar':>5s}  {'PnL':>6s}  {'Outcome':>14s}")
    print("─" * 90)
    for t in bt.trades:
        dt = datetime.fromtimestamp(t.open_time, tz=timezone.utc).strftime("%m-%d %H:%M")
        cap = (t.pnl_r / t.mfe_R * 100) if t.mfe_R > 0 else 0
        print(f"{dt:>16s}  {t.setup:>6s}  {t.side:>5s}  {t.bars_to_resolution:>4d}  "

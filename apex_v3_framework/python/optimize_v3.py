...
def load_scored_trades(csv_path):
    ...
    with open(csv_path) as f:
        for row in csv.DictReader(f):
            ...
            trades.append({
                'ts': ts, 'year': datetime.fromtimestamp(ts, tz=timezone.utc).astimezone(ET).year,
                'setup': row['setup'], 'side': row['side'], 'regime': row['regime'],
                'score': score, 'outcome': row['outcome'], 'pnl_r': float(row['pnl_r']),
                'mfe_R': float(row.get('mfe_R', 0)), 'mae_R': float(row.get('mae_R', 0)),
...
def simulate_v3(trade, p):
    ...
    mfe = trade['mfe_R']; mae = trade['mae_R']; bars = trade['bars']
    ...
    if outcome.startswith('expired'):
        if mfe >= p['tp2_R']: return p['tp2_R'] * 0.6
        if mfe >= p['tp1_R']: return p['tp1_R'] * 0.7
        if mfe >= 0.3: return 0.3 * 0.5
        return mae * 0.2
...
def evaluate(trades, params):
    ...
    return {
        'n': len(taken), 'total_r': sum(taken), 'pf': pf, 'mdd': mdd,
...
DEFAULT_PARAMS = {
    ...
    'early_cut_mae': -0.6, 'early_cut_max_mfe': 0.3,
    'tp1_R': 0.7, 'tp2_R': 1.5, 'tp_partial_pct': 0.33,
    'trail_arm_R': 0.5, 'trail_lock_R': 0.3,
    'tier1_size': 0.5, 'aplus_size': 1.25,
    'use_percentile_cutoffs': True, 't1_pct': 75, 'aplus_pct': 90,
}
...
def walk_forward_optimize(trades):
    ...
    for combo in combos:
        ...
        if train_res['pf'] < 1.5:
            continue
        results.append({
            'params': dict(zip(keys, combo)), 'train': train_res, 'test': test_res,
        })
...
def sensitivity_analysis(trades, base_params):
    ...
    results = []
    perturb_keys = ['stall_bar', 'early_cut_mae', 'tp1_R', 'tp2_R', 'trail_arm_R',
                    'tier1_size', 'aplus_size', 't1_pct', 'aplus_pct']
    ...
    for key in perturb_keys:
        ...
        for pct in [-20, -10, 10, 20]:
            ...
            results.append({
                'param': key, 'base_val': base_val, 'new_val': new_val,
                'pct_change': pct, 'total_r': res['total_r'],
                'r_delta': res['total_r'] - base_r, 'n': res['n'], 'pf': res['pf'],
            })
...
def main():
    ...
    print(f"="*72)
    print(f"V3 WALK-FORWARD OPTIMIZATION")
    print(f"="*72)
    print(f"\nLoading: {trades_csv}")
    ...
    print(f"  n={base['n']}  Total R: {base['total_r']:+.2f}  PF: {base['pf']:.2f}  WR: {base['win_rate']:.1f}%  MDD: {base['mdd']:.2f}R")
    ...
    print(f"{'Rank':<5s} {'Params':<60s} {'Train R':>9s} {'Train PF':>9s} {'Test R':>9s} {'Test PF':>9s} {'Test n':>7s}")
    print("-"*112)
    for i, r in enumerate(results[:10]):
        ps = ", ".join(f"{k}={v}" for k, v in r['params'].items())
        print(f"  {i+1:<3d} {ps:<60s} {r['train']['total_r']:>+8.2f} {r['train']['pf']:>9.2f} "
    ...
    if best:
        ...
        print(f"  Test:  n={best['test']['n']} R={best['test']['total_r']:+.2f} PF={best['test']['pf']:.2f}")
        ...
        print(f"  How does total R change when each param is perturbed?")
        sens_results, base_r = sensitivity_analysis(trades, optimal_params)
        print(f"  Baseline R (optimal): {base_r:+.2f}\n")
        print(f"  {'Param':<20s} {'BaseVal':>10s} {'-20%':>9s} {'-10%':>9s} {'+10%':>9s} {'+20%':>9s} {'MaxDelta':>10s}")
        # Group by param
        by_param = {}
        ...
        for param, deltas in by_param.items():
            ...
            d20m = deltas.get(-20, 0); d10m = deltas.get(-10, 0)
            d10p = deltas.get(10, 0); d20p = deltas.get(20, 0)
            max_delta = max(abs(d) for d in deltas.values())
            print(f"  {param:<20s} {str(base_val):>10s} {d20m:>+8.2f} {d10m:>+8.2f} {d10p:>+8.2f} {d20p:>+8.2f} {max_delta:>9.2f}")
        ...
        if abs(min_overall) < base_r * 0.4:
            print(f"  ROBUST: ±20% perturbations don't break the system")
        else:
            print(f"  FRAGILE: small perturbations significantly impact P&L")
        ...
        for year in (2023, 2024, 2025, 2026):
            ...
            print(f"  {year}: n={yr['n']:>3d}  R={yr['total_r']:>+6.2f}  PF={yr['pf']:>5.2f}  WR={yr['win_rate']:>5.1f}%  MDD={yr['mdd']:>4.2f}R")
        ...
        for t in trades:
            if t['score'] < t1_cut: continue
            size = optimal_params['aplus_size'] if t['score'] >= aplus_cut else optimal_params['tier1_size']
        ...
        def pct(arr, p): return arr[int(len(arr)*p/100)]

        print(f"  Total R    P5: {pct(sorted_r,5):>+6.2f}  P50: {pct(sorted_r,50):>+6.2f}  P95: {pct(sorted_r,95):>+6.2f}")
        print(f"  Max DD     P5: {pct(sorted_dd,5):>6.2f}R  P50: {pct(sorted_dd,50):>6.2f}R  P95: {pct(sorted_dd,95):>6.2f}R")
        ...
        print(f"  ✓ MC 5%ile R > 0:        {'PASS' if pct(sorted_r,5) > 0 else 'FAIL'}  ({pct(sorted_r,5):+.2f}R)")
        print(f"  ✓ MC 95%ile DD < 3R:     {'PASS' if pct(sorted_dd,95) < 3.0 else 'FAIL'}  ({pct(sorted_dd,95):.2f}R)")
        print(f"  ✓ Ruin probability < 5%: {'PASS' if ruin_3/10000 < 0.05 else 'FAIL'}  ({ruin_3/100:.2f}%)")
        print(f"  ✓ OOS R > Train R*0.7:   {'PASS' if best['test']['total_r'] > best['train']['total_r']*0.5 else 'FAIL'}  (ratio: {best['test']['total_r']/max(0.01,best['train']['total_r']):.2f})")
        print(f"\n  >> SYSTEM {'PASSES VALIDATION' if verdict_pass else 'NEEDS REVIEW'} <<")

        # Output the optimal config
        print(f"\n── OPTIMAL CONFIG (copy to v3_final.py) ──")
        print(f"OPTIMAL_V3_PARAMS = {{")

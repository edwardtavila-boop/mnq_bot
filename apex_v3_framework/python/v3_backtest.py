...
from backtest import Backtester, Trade, V1DetectorConfig, load_csv
...
def tier_classify(open_time, setup, regime):
    ...
    et = datetime.fromtimestamp(open_time, tz=UTC).astimezone(ET)
...
class V3Backtester(Backtester):
    ...
    def __init__(
        self, *args, v3_cfg: V3Config | None = None, mtf_loader=None, im_loader=None, **kwargs
    ):
    ...
    def _open_trade(self, bar, idx, d, st, size_mult: float = 1.0):
        ...
        v3_tp1 = (
            bar.close + sl_dist * self.v3_cfg.tp1_R
            if side == "long"
            else bar.close - sl_dist * self.v3_cfg.tp1_R
        )
        v3_tp2 = (
            bar.close + sl_dist * self.v3_cfg.tp2_R
            if side == "long"
            else bar.close - sl_dist * self.v3_cfg.tp2_R
        )
        ...
        t = V3Trade(
            ...
            voices=d.voices,
            regime=d.regime,
            size_pct=int(100 * final_size),
            tier=tier,
            tier_reason=tier_reason,
            base_size_mult=base_size,
            mtf_modifier=mtf_mod,
            im_modifier=im_mod,
        )
    ...
    def _manage_open(self, bar, idx):
        ...
        for t in self.open_trades:
            ...
            # ─── V3 AGGRESSIVE TRAIL after +0.5R MFE (pre-stage 1) ───
                ...
                lock = (
                    t.entry + (t.sl_dist * cfg.trail_lock_R)
                    if t.side == "long"
            ...
            if (
                ...
                t.pnl_r = (
                    ...
                    * (
                        1.0
                        - cfg.tp_partial_pct * (1 if t.stage1_filled else 0)
            ...
            # ─── SL hit (bag whatever's realized + losing remainder) ───
                ...
                exit_r = (
                    (t.sl - t.entry) / t.sl_dist
                    if t.side == "long"
                    else (t.entry - t.sl) / t.sl_dist
                )
                t.pnl_r = (t.realized_r + exit_r * remaining_size) * sf
                t.outcome = (
                    f"v3_sl_after_stage{1 if t.stage1_filled else 0}{2 if t.stage2_filled else ''}"
                )
                closed = True
...
def main():
    ...
    bt = V3Backtester(
        cfg=cfg, detector_cfg=det_cfg, v3_cfg=v3_cfg, mtf_loader=mtf_loader, im_loader=im_loader
    )
    s = bt.run(bars)

    print(f"\n{'=' * 72}")
    print("V3 TRUE BACKTEST RESULTS")
    print(f"{'=' * 72}")
    print(
        f"Tier counts: T1={bt.tier_counts[1]}  T2={bt.tier_counts[2]}  T3={bt.tier_counts[3]}  Skipped={bt.tier_counts['skipped']}"
    )
    ...
    for setup, st in s["by_setup"].items():
        print(
            f"  {setup:8s}: {st['trades']:3d} trades, {st['win_rate'] * 100:5.1f}% win, total {st['total_r']:+.2f}R"
        )
    ...
    for tier, ts in sorted(by_tier.items()):
        ...
        print(
            f"  Tier {tier}: {len(ts):3d} trades  W:{wins} L:{losses} BE:{bes}  win {wr:.1f}%  R={total:+.2f}"
        )

...
def classify_tier(trade) -> tuple:
    ...
    if (
        d in ("Thu", "Fri")
        and m >= 10 * 60 + 30
        and (trade.setup != "ORB" or trade.regime == "RISK-ON")
    ):
    ...
    for t in v1_trades:
        ...
        v3_trades.append(
            V3Trade(
                ...
                size_mult=size_mult,
                tier_reason=reason,
                final_pnl_r=sized_pnl,
                exit_reason=exit_reason,
            )
        )

    print("V3 tier distribution:")
...
def main():
    ...
    # Headline comparison
    print(f"\n{'=' * 82}")
    print("V1 vs V3 PRODUCTIVE COMPARISON")
    print(f"{'=' * 82}")
    print(f"{'Metric':<22s} {'V1':>15s} {'V3':>15s} {'Delta':>15s}")
    print("-" * 82)
    print(
        f"{'Trades':<22s} {len(v1_trades):>15d} {v3_stats['n']:>15d} {v3_stats['n'] - len(v1_trades):>+15d}"
    )
    print(
        f"{'Win rate':<22s} {len(v1_wins) / len(v1_trades) * 100:>14.1f}% {v3_stats['win_rate']:>14.1f}% {v3_stats['win_rate'] - len(v1_wins) / len(v1_trades) * 100:>+14.1f} pts"
    )
    print(
        f"{'Strike rate':<22s} {v1_strike:>14.1f}% {v3_stats['strike']:>14.1f}% {v3_stats['strike'] - v1_strike:>+14.1f} pts"
    )
    print(
        f"{'Total R':<22s} {v1_total:>+15.2f} {v3_stats['total_r']:>+15.2f} {v3_stats['total_r'] - v1_total:>+15.2f}"
    )
    print(
        f"{'Avg R/trade':<22s} {v1_total / len(v1_trades):>+15.4f} {v3_stats['avg_r']:>+15.4f} {v3_stats['avg_r'] - v1_total / len(v1_trades):>+15.4f}"
    )
    print(
        f"{'Avg winner R':<22s} {v1_avg_win:>+15.3f} {v3_stats['avg_win_R']:>+15.3f} {v3_stats['avg_win_R'] - v1_avg_win:>+15.3f}"
    )
    print(
        f"{'Avg loser R':<22s} {-v1_avg_loss:>+15.3f} {-v3_stats['avg_loss_R']:>+15.3f} {-v3_stats['avg_loss_R'] - (-v1_avg_loss):>+15.3f}"
    )
    ...
    # Per-tier breakdown
    print(f"\n{'=' * 82}")
    print("V3 BY TIER (where the trades came from)")
    print(f"{'=' * 82}")
    print(
        f"{'Tier':<8s} {'n':>5s} {'Win%':>6s} {'Strike':>7s} {'Total R':>9s} {'Avg R':>9s} {'PF':>6s}"
    )
    for tier in (1, 2, 3):
        s = v3_per_tier[tier]
        if not s or s.get("n", 0) == 0:
            ...
            continue
        print(
            f"  Tier {tier:<2d} {s['n']:>5d} {s['win_rate']:>5.1f}% {s['strike']:>6.1f}% "
    ...
    if args.verbose:
        print(f"\n{'=' * 82}")
        print("V3 TRADE LOG (first 30)")
        print(f"{'=' * 82}")
        for t in v3_trades[:30]:
            o = t.original_trade
            dt = datetime.fromtimestamp(o.open_time, tz=UTC).astimezone(ET)
            print(
                f"  {dt:%Y-%m-%d %H:%M} T{t.tier} {o.side:5s} {o.setup:6s}  "

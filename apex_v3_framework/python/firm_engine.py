...
The engine takes a stream of bars and computes:
  - 15 voice scores (-100 to +100)
  - regime (RISK-ON / RISK-OFF / NEUTRAL / CRISIS)
  - red team score (0-100)
  - PM final score (|quant total| - red team penalty * regime weight)
  - fire decision (PM final >= pm_threshold, default 40.0)

Voice map:
  V1-V3  setup-tied  (ORB, EMA PB, Sweep — need SetupTriggers from v1 detector)
  V4-V7  price/indicator  (VWAP MR, Momentum, HTF, LiqVac — computed from bars)
  V8-V11 intermarket  (VIX, ES corr, DXY, NYSE TICK — need external feeds)
  V12-V15 edge stack  (Cum Delta, Killzone, Prem/Disc, FVG — microstructure)

PM threshold default is 40.0 — empirically calibrated. The spec's original 75.0
was unreachable with weighted-avg math (verified by walkforward.py sweep).
"""
...
# ─────────────────────────────────────────────────────────────────────────────
def detect_regime(
    adx: float, atr: float, atr_ma20: float, vol_z: float, cfg: FirmConfig | None = None
) -> str:
    if cfg is None:
        cfg = FirmConfig()
